...
const DAILY_EXPORT_FRIENDLY_ERR: &str =
    "Export failed. See Settings → Diagnostics → logs for details.";

/// "2026-06-30" → "Monday, June 30, 2026"; falls back to the raw ISO date if
/// the string doesn't parse.
fn format_recap_day_heading(iso: &str) -> String {
...
/// Render one Markdown document from a set of recap rows (newest first).
/// Non-`Generated` rows (quiet/failed days) are skipped.
fn render_daily_summaries_markdown(
    rows: &[DailySummaryRow],
    range_label: &str,
    exported_at: &str,
) -> String {
    use crate::daily_summary::generator::{SectionItem, Sections};

    fn render_section(out: &mut String, title: &str, items: &[SectionItem]) {
        if items.is_empty() {
            return;
        }
        out.push_str(&format!("\n### {title}\n\n"));
        for it in items {
            match it.source_id.as_deref() {
    ...
    out.push_str(&format!("# Daily recap — {range_label}\n\n"));
    out.push_str(&format!("_Exported {exported_at}_\n"));

    let mut first = true;
    for row in rows {
        if !matches!(row.status, SummaryStatus::Generated) {
            continue;
        }
        out.push_str(if first { "\n" } else { "\n---\n\n" });
        first = false;

        out.push_str(&format!("## {}\n\n", format_recap_day_heading(&row.date)));
        let narrative = row.narrative.trim();
        if !narrative.is_empty() {
            out.push_str(narrative);
            out.push('\n');
        }
        let sections: Sections = serde_json::from_str(&row.sections_json).unwrap_or_default();
        render_section(&mut out, "Meetings", &sections.meetings);
        render_section(&mut out, "Focus work", &sections.focus_work);
        render_section(&mut out, "Notes", &sections.notes);
        render_section(
            &mut out,
            "Things that came up",
            &sections.things_that_came_up,
        );
    }
...
/// Export stored daily recaps whose date falls within the inclusive range
/// `[since_date, until_date]` (`YYYY-MM-DD`, local dates computed by the
/// frontend) to a single Markdown file in Downloads, then reveal it in Finder.
#[tauri::command]
pub fn export_daily_summaries(
    ...
    let rows = db
        .with_conn(|c| {
            daily_summaries_db::list_in_range(c, &since_date, &until_date)
        ...
        .map_err(|e| {
            error!(
                target: "export",
                error = %format!("{e:?}"),
                "export_daily_summaries: query failed"
            );
            DAILY_EXPORT_FRIENDLY_ERR.to_string()
        })?;

    let generated_count = rows
        .iter()
        .filter(|r| matches!(r.status, SummaryStatus::Generated))
        .count();
    if generated_count == 0 {
        info!(
            ...
            since = %since_date,
            until = %until_date,
            "export_daily_summaries: no recaps in range"
        );
        return Err("No recaps to export in this range.".into());
    }

    let now = chrono_now_iso();
    let body = render_daily_summaries_markdown(&rows, &range_label, &now);

    let dir = app.path().download_dir().map_err(|e| {
        error!(target: "export", error = %e, "export_daily_summaries: no Downloads dir");
        DAILY_EXPORT_FRIENDLY_ERR.to_string()
    })?;
    // "2026-06-30T18:04:21Z" → "2026-06-30-1804"
    let stamp = format!(
        "{}-{}{}",
        now.get(0..10).unwrap_or("0000-00-00"),
        now.get(11..13).unwrap_or("00"),
    ...
    let path = dir.join(format!("echo-scribe-daily-{stamp}.md"));
    std::fs::write(&path, &body).map_err(|e| {
        error!(
            target: "export",
            path = %path.display(),
            error = %e,
            "export_daily_summaries: write failed"
        );
        DAILY_EXPORT_FRIENDLY_ERR.to_string()
    })?;
