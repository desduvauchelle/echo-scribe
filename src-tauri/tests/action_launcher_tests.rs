use echo_scribe_lib::llm::action_launcher::{
    detect_action, normalize_spoken_email, strip_trigger_prefix,
};
...
async fn test_detect_action_retry_logic() {
    ...
    let cmd = detect_action(&mock, "some random voice dictation", &[])
        .await
        .unwrap();
...
async fn test_detect_action_failed_twice_returns_error() {
    ...
    let mock = MockLlm {
        responses: Arc::new(Mutex::new(vec![
            invalid_json_1.to_string(),
            invalid_json_2.to_string(),
        ])),
        requests: Arc::new(Mutex::new(vec![])),
    };
...
fn test_normalize_spoken_email() {
    ...
    assert_eq!(
        normalize_spoken_email("test at example.com"),
        "test@example.com"
    );
    assert_eq!(
        normalize_spoken_email("john [at] github.com"),
        "john@github.com"
    );
    assert_eq!(
        normalize_spoken_email("admin (at) livecase.org"),
        "admin@livecase.org"
    );
    ...
    assert_eq!(
        normalize_spoken_email("denis@livecase.com"),
        "denis@livecase.com"
    );

    // Test that a normal email with single hyphen remains unaffected
    assert_eq!(
        normalize_spoken_email("jean-paul@example.com"),
