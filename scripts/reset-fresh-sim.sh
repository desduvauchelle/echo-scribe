#!/usr/bin/env bash
# Reset "Tucky Fresh" back to a never-launched state:
# wipe its (isolated) data folders and its TCC permission grants.
# The real Tucky app, data, models and permissions are untouched.
set -euo pipefail
...
rm -f  "$HOME/Library/LaunchAgents/Tucky Fresh.plist"
...
echo "Tucky Fresh reset to fresh-install state (data + permissions cleared)."
